...
export default function GuideTable({ guides, onCertify, onDelete }) {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/park-guides/${guideToDelete.guide_id}`, {
        method: "DELETE",
      });
    ...
    <>
      <div className="bg-white rounded-xl shadow-md border border-green-300 overflow-x-auto">
        ...
        <div className="flex border-b bg-green-50">
          {["certification", "transaction", "allGuides"].map((tab) => (
            <button
              key={tab}
              ...
              <tbody className="text-green-900">
                {guides.map((guide) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{guide.email}</td>
                    <td className="px-4 py-2">{guide.user_status}</td>
                    <td className="px-4 py-2">{guide.assigned_park || "N/A"}</td>
                    <td className="px-4 py-2">
                      <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md text-center space-y-4">
            ...
            <p className="text-gray-700">
              ...
              <strong>{guideToDelete?.first_name} {guideToDelete?.last_name}</strong>?
              This will remove both their park guide and user record.
            </p>
