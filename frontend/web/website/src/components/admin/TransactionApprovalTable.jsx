...
export default function TransactionApprovalTable() {
  ...
  const handleApprove = async (paymentId) => {
    ...
    try {
      ...
      setTransactions((prev) => prev.filter((tx) => tx.payment_id !== paymentId));
    } catch (err) {
    ...
    <div className="bg-white rounded-xl shadow-md border border-green-300 overflow-x-auto">
      ...
      {loading ? (
        ...
        <table className="min-w-full text-sm text-left text-green-900">
          ...
          <tbody>
            {transactions.map((tx) => (
              <tr key={tx.payment_id} className="border-t hover:bg-green-50 transition">
                <td className="px-4 py-2">{tx.payment_id}</td>
                <td className="px-4 py-2">{tx.user_name}</td>
                <td className="px-4 py-2">{tx.user_email}</td>
                <td className="px-4 py-2">{tx.paymentPurpose}</td>
                <td className="px-4 py-2">{tx.paymentMethod}</td>
                <td className="px-4 py-2">RM {Number(tx.amountPaid).toFixed(2)}</td>
