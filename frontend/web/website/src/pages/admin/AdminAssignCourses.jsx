...
export default function AdminAssignCourses() {
  ...
  const handleDetails = async (guide) => {
    ...
    try {
      ...
      const fetchFeedback = axios
        .get(`${API_URL}/api/course_feedback?guide_id=${guide.guide_id}`, {
      ...
      const fetchRecommendations = axios
        .get(`${API_URL}/api/recommendations?guide_id=${guide.guide_id}`, {
          headers: { "ngrok-skip-browser-warning": "true" },
        })
        .then((res) => {
          if (res.data?.success) {
  ...
  const handleDeleteInProgressCourses = async () => {
    if (
      !selectedGuide ||
      !window.confirm(
        `Do you sure to delete ${selectedGuide.first_name} in progress courses? Completed courses will be remain`
      )
    ) {
      return;
    }
    try {
      setDeleting(true);
      const response = await axios.delete(`${API_URL}/api/assign_course`, {
    ...
    const loadData = async () => {
      try {
        ...
        const [guidesRes, modulesRes] = await Promise.all([
          axios.get(`${API_URL}/api/assign_course?type=guides`, {
            headers: { "ngrok-skip-browser-warning": "true" },
          }),
          axios.get(`${API_URL}/api/assign_course?type=modules`, {
            headers: { "ngrok-skip-browser-warning": "true" },
          }),
        ]);
          ...
          {detailTab === "comments" && (
            <div className="space-y-4 max-h-96 overflow-y-auto">
              <h4 className="font-bold text-lg mb-2">
                Visitor Feedback ({feedbackData?.feedback_count || 0})
              </h4>
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between mb-3">
                        <h5 className="text-lg font-semibold capitalize text-gray-800">
                          {rec.aspect === "language"
                            ? "Language Skills"
                            : rec.aspect === "knowledge"
                            ? "Knowledge"
                            : rec.aspect === "organization"
                            ? "Organizational Skills"
                            : rec.aspect === "engagement"
                            ? "Engagement Skills"
                            : "Safety Awareness"}
                        </h5>
                      ...
                      {rec.modules && rec.modules.length > 0 ? (
                        ...
                        <div className="bg-yellow-50 p-3 rounded-lg text-center">
                          <p className="text-yellow-700 flex items-center justify-center">
                            ...
                            No suitable {rec.level} level courses available at
                            the moment
                          </p>
                        </div>
                      )}
                ...
                >
                  ...
                  {modules
                    ...
                    .map((m) => (
                      ...
                      >
                        {m.difficulty.charAt(0).toUpperCase() +
