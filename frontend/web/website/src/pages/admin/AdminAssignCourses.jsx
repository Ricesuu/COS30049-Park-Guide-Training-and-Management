...
export default function AdminAssignCourses() {
    ...
    const handleDetails = async (guide) => {
        ...
        try {
            ...
            const fetchFeedback = axios
                .get(`/api/course_feedback?guide_id=${guide.guide_id}`)
                .then((res) => {
                    if (res.data.success) {
            ...
            const fetchRecommendations = axios
                .get(`/api/recommendations?guide_id=${guide.guide_id}`)
                .then((res) => {
                    if (res.data?.success) {
            ...
            const [feedbackSuccess, recommendationSuccess] = await Promise.all([
                fetchFeedback,
    ...
    const handleDeleteInProgressCourses = async () => {
        ...
        if (!selectedGuide.assigned_modules?.length) {
            alert("This guide has no courses in progress");
            return;
        }

        if (
            !window.confirm(
                `Are you sure you want to delete all in-progress courses for ${selectedGuide.first_name}? Completed courses will remain.`
            )
        ) {
            return;
        }
    ...
    const handleAssign = async () => {
        // Check if guide is selected and has proper data
        if (!selectedGuide?.guide_id) {
            alert("Please select a valid guide first");
            return;
        }

        // Filter out empty selections
        const moduleIds = Object.values(selectedModules).filter(
            (id) => id != null && id !== ""
        );
        ...
        <div className="p-6">
            ...
            {viewMode === "details" && selectedGuide && (
                <div className="bg-white shadow rounded-lg p-6 mt-6 border border-gray-200">
                    ...
                    {detailTab === "aspects" && (
                        <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                            ...
                            <div className="text-3xl font-bold text-blue-600">
                                {feedbackData?.averages?.total?.toFixed(1) ||
                                    "0.0"}
                            </div>
                            <p className="text-gray-600 mt-2">
                                Based on {feedbackData?.feedback_count || 0}{" "}
                                feedbacks
                            </p>
                            <ul className="mt-4 space-y-3">
                                {aspects.map((aspect) => {
                                    const rating =
                                        feedbackData?.averages?.[aspect] || 0;
                            ...
                            {feedbackData?.feedbacks?.length > 0 ? (
                                ...
                                <div className="text-center py-6">
                                    ...
                                    <p className="text-sm text-gray-400 mt-1">
                                        Visitors haven't left any feedback
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white p-4 rounded-lg shadow">
                            <h4 className="font-bold text-xl mb-4 text-teal-800 border-b pb-2">
                                ...
                                <span className="block text-sm font-normal text-gray-500 mt-1">
                                    ...
                                    {selectedGuide?.first_name}'s performance
                                </span>
                            </h4>
                    ...
                    {/* error handling */}
                    {!["aspects", "comments", "recommend"].includes(
                        detailTab
                    ) && (
            ...
            {selectedGuide && (
                <div className="bg-white shadow rounded-lg p-6 mt-6 border border-gray-200">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h3 className="text-lg font-semibold">
                                Assigning courses to: {selectedGuide.first_name}{" "}
                                {selectedGuide.last_name}
                            </h3>
                        ...
                        <button
                            ...
                            disabled={
                                deleting ||
                                !selectedGuide.assigned_modules?.length
                            }
                            className={`px-3 py-1 rounded text-sm ${
                                deleting
                                    ? "bg-gray-300"
                                    ...
                                    {modules
                                        ...
                                        .map((m) => (
                                            <option
                                                ...
                                                value={m.module_id}
                                                className={
                                                    selectedGuide.assigned_modules?.includes(
                                                        m.module_id
                                                    )
                                                        ? "bg-blue-50"
                                                        : ""
