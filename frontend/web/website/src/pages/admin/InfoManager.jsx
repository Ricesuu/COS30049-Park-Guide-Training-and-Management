...
export default function PlantManager() {
  ...
  const handleSavePlant = async (plantData) => {
    try {
      ...
      if (plantData.plant_id) {
        ...
        response = await fetch(`${API_BASE_URL}/plants/${plantData.plant_id}`, {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        try {
          ...
          throw new Error(
            errorData.error || "Failed to save plant information"
          );
        } catch (parseError) {
          // If we can't parse the error as JSON, use the status text
          throw new Error(
            `Failed to save plant information: ${response.statusText}`
          );
        }
      }
  ...
  const handleDeletePlant = async (plantId) => {
    if (
      window.confirm("Are you sure you want to delete this plant information?")
    ) {
      try {
        ...
        if (!response.ok) {
          try {
            const errorData = await response.json();
            throw new Error(
              errorData.error || "Failed to delete plant information"
            );
          } catch (parseError) {
            // If we can't parse the error as JSON, use the status text
            throw new Error(
              `Failed to delete plant information: ${response.statusText}`
            );
          }
        }
  ...
  if (loading) {
    return (
      <div className="p-4 text-green-950">Loading plant information...</div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg shadow-sm border border-green-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-green-800 text-xl font-semibold mb-1">
                Total Plants
              </h2>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-lg shadow-sm border border-blue-200">
            <div className="flex items-center justify-between">
              {(() => {
                ...
                const newestPlant = [...plants].reduce(
                  (max, plant) => (plant.plant_id > max.plant_id ? plant : max),
                  plants[0]
                );
                  ...
                  <>
                    <div>
                      ...
                      <p className="text-blue-700 font-medium">
                        {newestPlant.common_name}
                      </p>
                      <p className="text-blue-600 text-sm italic">
                        {newestPlant.scientific_name}
                      </p>
                    </div>
                    <div className="bg-white p-2 rounded-md shadow-md h-16 w-16 flex items-center justify-center">
                      {newestPlant.image_url ? (
                        <img
                          ...
                          onError={(e) => {
                            e.target.src =
                              "https://via.placeholder.com/50?text=Plant";
                          }}
                        />
                      ) : (
        ...
        {plants.length > 0 && (
          <div className="bg-gradient-to-r from-amber-50 to-amber-100 p-4 rounded-lg shadow-sm border border-amber-200">
            <div className="flex flex-col">
              <h2 className="text-amber-800 text-xl font-semibold mb-1">
                Plant Families
              </h2>
          ...
          <table className="border-collapse rounded w-full table-auto">
            <thead>
              <tr className="border-b-2 border-green-800 bg-green-50">
                <th className="text-left py-3 px-4 text-green-800">ID</th>
                ...
                <th className="text-left py-3 px-4 text-green-800">Family</th>
                <th className="text-left py-3 px-4 text-green-800">
                  Description
                </th>
                <th className="text-left py-3 px-4 text-green-800">Image</th>
            ...
            <tbody>
              {plants.map((plant) => (
                <tr key={plant.plant_id} className="hover:bg-gray-50">
                  <td className="py-3 px-4">{plant.plant_id}</td>
                  <td className="py-3 px-4">{plant.common_name}</td>
                  <td className="py-3 px-4 italic">{plant.scientific_name}</td>
                  <td className="py-3 px-4">{plant.family}</td>
