  ...
  useEffect(() => {
    const fetchModuleData = async () => {
      try {
        ...
        const token = await user.getIdToken(); // Fetch module data including course_content and video_url
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            throw new Error(
              errorData.error || "Access to this module is restricted"
            );
          }
        ...
        const quizResponse = await fetch(
          `${API_URL}/api/quizattempts?moduleId=${moduleId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
