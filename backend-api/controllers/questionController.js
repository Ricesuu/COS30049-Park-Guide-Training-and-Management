...
export async function getQuizQuestions(req, quizId) {
  try {
    ...
    const [options] = await conn.query(
      "SELECT * FROM options WHERE question_id IN (?) ORDER BY id",
      [questionIds]
    );
...
export async function getQuestionById(req, id) {
  try {
    ...
    const [options] = await conn.query("SELECT * FROM options WHERE question_id = ?", [id]);
...
export async function createQuestion(req, quizId) {
  ...
  if (!options || !Array.isArray(options) || options.length === 0) {
    return NextResponse.json({ message: "Question options are required" }, { status: 400 });
  }

  if (type === "multiple-choice" && !options.some((opt) => opt.isCorrect)) {
    return NextResponse.json({ message: "At least one correct option is required" }, { status: 400 });
  }
  ...
  try {
    ...
    const [questions] = await conn.query("SELECT * FROM questions WHERE id = ?", [questionId]);
    const [insertedOptions] = await conn.query("SELECT * FROM options WHERE question_id = ?", [questionId]);

    return NextResponse.json({ ...questions[0], options: insertedOptions }, { status: 201 });
  } catch (error) {
    await conn.rollback();
...
export async function updateQuestion(req, id) {
  ...
  try {
    ...
    const [updateResult] = await conn.query(
      "UPDATE questions SET type = ?, text = ?, explanation = ?, points = ? WHERE id = ?",
    ...
    const [questions] = await conn.query("SELECT * FROM questions WHERE id = ?", [id]);
    const [updatedOptions] = await conn.query("SELECT * FROM options WHERE question_id = ?", [id]);
...
export async function duplicateQuestion(req, id) {
  ...
  try {
    ...
    const [duplicateResult] = await conn.query(
      "INSERT INTO questions (quiz_id, type, text, explanation, points) VALUES (?, ?, ?, ?, ?)",
      [question.quiz_id, question.type, `${question.text} (copy)`, question.explanation, question.points]
    );
    ...
    const [newQuestions] = await conn.query("SELECT * FROM questions WHERE id = ?", [newQuestionId]);
    const [newOptions] = await conn.query("SELECT * FROM options WHERE question_id = ?", [newQuestionId]);

    return NextResponse.json({ ...newQuestions[0], options: newOptions }, { status: 201 });
  } catch (error) {
    await conn.rollback();
