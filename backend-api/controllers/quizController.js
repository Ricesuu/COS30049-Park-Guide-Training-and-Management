...
export async function getAllQuizzes(req) {
  try {
    ...
    const [rows] = await conn.query(`
      SELECT q.quiz_id, q.name, q.description, 
             COUNT(qn.question_id) AS questionCount, 
...
export async function getQuizById(req, id) {
  try {
    const conn = await getConnection();
    const [rows] = await conn.query("SELECT * FROM quizzes WHERE quiz_id = ?", [
      id,
    ]);
...
export async function deleteQuiz(req, id) {
  try {
    const conn = await getConnection();
    const [result] = await conn.query("DELETE FROM quizzes WHERE quiz_id = ?", [
      id,
    ]);
    conn.release();
