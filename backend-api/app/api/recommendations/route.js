...
export async function GET(request) {
  ...
  try {
    ...
    const recommendations = ['language', 'knowledge', 'organization', 'engagement', 'safety']
      .map(aspect => {
        const score = parseFloat(stats[0][aspect]) || 0;
        const level = STRICT_SCORE_LEVEL_MAP.find(r => score >= r.min && score <= r.max).level;

        const recommendedModules = modules.filter(module =>
          module.aspect === aspect &&
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        hasAnyRecommendations: recommendations.some(r => r.modules.length > 0)
      }
    });
