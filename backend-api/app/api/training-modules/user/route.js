...
export async function GET(request) {
    ...
    try {
        ...
        if (rows.length === 0) {
            console.log(
                `No module purchases found for user ${userId}. Checking for pending payments with modules...`
            );
            ...
            const [pendingModules] = await connection.execute(
                ...
                SELECT 
                    ...
                    CONCAT('${
                        ...
                        "http://localhost:3001"
                    }/module-images/', pt.module_id, '.jpg') AS imageUrl,
                    CONCAT('${
                        process.env.NEXT_PUBLIC_API_URL ||
                        "http://localhost:3001"
            ...
            if (pendingModules.length > 0) {
                console.log(
                    `Found ${pendingModules.length} pending payments with modules. Creating ModulePurchases records...`
                );
                    ...
                    try {
                        await connection.execute(
                            ...
                            [
                                userId,
                                trainingModule.id,
                                trainingModule.payment_id,
                            ]
                        );
                        console.log(
                            `Created ModulePurchases record for moduleId: ${trainingModule.id}, userId: ${userId}, paymentId: ${trainingModule.payment_id}`
                        );
                    } catch (err) {
                        console.error(
                            `Error creating ModulePurchases record:`,
                            err
                        );
                    }
                }
