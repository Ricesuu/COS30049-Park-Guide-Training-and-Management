...
export async function GET(request, { params }) {
  ...
  try {
    ...
    // If module is not premium, access is granted
    if (!modules[0].is_premium) {
      return NextResponse.json({ hasAccess: true, reason: "free_module" });
    }
    
    // For premium modules, check if user has purchased it
