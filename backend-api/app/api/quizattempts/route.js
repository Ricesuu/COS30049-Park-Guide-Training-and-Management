    ...
    try {
        ...
        if (!moduleId || !Array.isArray(selectedAnswers) || selectedAnswers.length === 0 || !Array.isArray(question_ids)) {
            return NextResponse.json(
                { error: "Missing or invalid moduleId, selectedAnswers, or question_ids" },
                { status: 400 }
            );
        }
        ...
        let score = 0;
        selectedAnswers.forEach((selectedOptionId, index) => {
            const questionId = question_ids[index];
            const correct = correctMap.get(questionId);
            if (correct && correct === selectedOptionId) {
                score++;
            }
        });
        ...
        try {
            ...
            for (let i = 0; i < selectedAnswers.length; i++) {
                ...
                const isCorrect = correctMap.get(questionId) === selectedOptionId;
                
                await connection.execute(
                    `INSERT INTO quizresponses 
                     (attempt_id, question_id, selected_option_id, is_correct, time_taken, answer_sequence)
                     VALUES (?, ?, ?, ?, ?, ?)`,
                    [attemptId, questionId, selectedOptionId, isCorrect ? 1 : 0, answerTimes[i] || 0, i + 1]
                );
            }
